...
# These plain schemas are then used within the original schemas to break
# the potential infinite loop caused by nested relationships.
...
class UserSchema(PlainUserSchema):
    # read-only fields are marked with dump_only=True
...
class TeamSchema(PlainTeamSchema):
    ...
    # can only be modified with PUT/POST requests, not output with GET
